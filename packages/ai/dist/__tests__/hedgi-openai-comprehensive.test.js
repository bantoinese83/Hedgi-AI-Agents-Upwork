...
describe('HedgiOpenAI - Comprehensive Tests', () => {
    ...
    beforeEach(() => {
        ...
        openai = (0, hedgi_openai_1.createHedgiOpenAI)({
            apiKey: 'test-api-key',
            enableCostLogging: true,
        });
        // Clear cache between tests
        openai.responseCache.clear();
    ...
    describe('Constructor and Configuration', () => {
        it('should create instance with valid config', () => {
            expect(openai).toBeInstanceOf(hedgi_openai_1.HedgiOpenAI);
        });
        it('should throw error for missing API key', () => {
            expect(() => {
                (0, hedgi_openai_1.createHedgiOpenAI)({
                    apiKey: '',
                    enableCostLogging: true,
                });
    ...
    describe('Token Validation', () => {
        it('should validate token limits correctly', () => {
            ...
            const shortValidation = openai.validateTokenLimits(shortSystemPrompt, shortUserPrompt);
            const longValidation = openai.validateTokenLimits(longSystemPrompt, longUserPrompt);
    ...
    describe('Cost Calculation', () => {
        it('should calculate costs correctly', () => {
            ...
            const cost = openai.calculateCost(tokenUsage);
        ...
        it('should calculate costs for different models', () => {
            ...
            const cost = openai.calculateCost(tokenUsage, 'gpt-4-0613');
    ...
    describe('Payload Pruning', () => {
        ...
        it('should sort transactions by materiality', () => {
            ...
            const pruned = openai.prunePayload(payload);
            const transactions = pruned.transactions;
    ...
    describe('Cache Functionality', () => {
        it('should cache responses', async () => {
            ...
            // First call
            const result1 = await openai.callWithJSONMode('smb-explainer', 'Test system prompt', 'Test user prompt', schemas_1.SMBExplainerResponseSchema, input);
            // Second call should use cache
            const result2 = await openai.callWithJSONMode('smb-explainer', 'Test system prompt', 'Test user prompt', schemas_1.SMBExplainerResponseSchema, input);
        ...
        it('should generate different cache keys for different inputs', async () => {
            ...
            openai.responseCache.clear();
            await openai.callWithJSONMode('smb-explainer', 'Test system prompt', 'Test user prompt', schemas_1.SMBExplainerResponseSchema, input1);
            // Clear cache before second call
            openai.responseCache.clear();
            await openai.callWithJSONMode('smb-explainer', 'Test system prompt', 'Test user prompt', schemas_1.SMBExplainerResponseSchema, input2);
    ...
    describe('JSON Mode with Retry', () => {
        it('should succeed on first attempt with valid JSON', async () => {
            ...
            const result = await openai.callWithJSONMode('smb-explainer', 'Test system prompt', 'Test user prompt', schemas_1.SMBExplainerResponseSchema, input);
            expect(result.success).toBe(true);
            expect(result.data.data.summary).toBe('Test summary');
            expect(result.data.data.key_insights).toEqual(['Test insight']);
            expect(result.data.data.recommendations).toEqual(['Test recommendation']);
            expect(result.data.data.financial_health_score).toBe(85);
            expect(mockCreate).toHaveBeenCalledTimes(1);
        });
        ...
        it('should fail after max retries', async () => {
            ...
            await expect(openai.callWithJSONMode('smb-explainer', 'Test system prompt', 'Test user prompt', schemas_1.SMBExplainerResponseSchema, input, 1 // Max 1 retry
        ...
        it('should handle missing content in response', async () => {
            ...
            await expect(openai.callWithJSONMode('smb-explainer', 'Test system prompt', 'Test user prompt', schemas_1.SMBExplainerResponseSchema, input)).rejects.toThrow('No content in OpenAI response');
        });
    });
    describe('Error Handling', () => {
        it('should handle OpenAI API errors', async () => {
            ...
            await expect(openai.callWithJSONMode('smb-explainer', 'Test system prompt', 'Test user prompt', schemas_1.SMBExplainerResponseSchema, input)).rejects.toThrow('OpenAI API Error');
        });
        it('should handle missing content in response', async () => {
            mockCreate.mockResolvedValue({
                choices: [{}], // No message content
    ...
    describe('Cost Logging', () => {
        it('should log costs when enabled', async () => {
            ...
            await openai.callWithJSONMode('smb-explainer', 'Test system prompt', 'Test user prompt', schemas_1.SMBExplainerResponseSchema, input);
            // Cost should be logged (mocked logger will be called)
            expect(true).toBe(true); // Test passes if no error thrown
        });
        it('should not log costs when disabled', async () => {
            const noLoggingOpenai = (0, hedgi_openai_1.createHedgiOpenAI)({
                apiKey: 'test-api-key',
            ...
            await noLoggingOpenai.callWithJSONMode('smb-explainer', 'Test system prompt', 'Test user prompt', schemas_1.SMBExplainerResponseSchema, input);
    ...
    describe('Utility Methods', () => {
        it('should get row count from payload', () => {
            ...
            const rowCount = openai.getRowCount(payload);
        ...
        it('should get data types from payload', () => {
            ...
            const dataTypes = openai.getDataTypes(payload);
...
//# sourceMappingURL=hedgi-openai-comprehensive.test.js.map